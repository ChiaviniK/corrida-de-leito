...
export default function App() {
  // Estados de Autenticação e Controle de Acesso
  ...
  const [autor, setAutor] = useState(() => {
    return localStorage.getItem('corrida_autor') || 'Augusto';
  });
  ...
  // Validação da Sessão / Token do Usuário ao carregar
  useEffect(() => {
    ...
    fetch('/api/auth/me', {
      ...
      .then((user) => {
        ...
        setAutor(user.nome || user.username);
        carregarLeitos();
      })
      .catch(() => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      localStorage.setItem('corrida_token', data.access_token);
      setUsuarioLogado(data.usuario);
      setAutor(data.usuario.nome || data.usuario.username);
      setLoginPassword('');
      dispararToast(`Bem-vindo, ${data.usuario.nome}!`);
      carregarLeitos();
    } catch (err) {
  ...
  const handleLogout = async () => {
    ...
    localStorage.removeItem('corrida_token');
    setUsuarioLogado(null);
    dispararToast('Sessão finalizada com sucesso.');
  };
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-100 dark:bg-slate-950 text-slate-600 dark:text-slate-300">
        <div className="flex flex-col items-center gap-3">
          ...
          <span className="text-xs font-semibold">Verificando credenciais hospitalares...</span>
        </div>
      </div>
    );
  }

  // Tela de Login Obrigatória (Acesso Restrito)
  if (!usuarioLogado) {
    return (
      <div className="min-h-screen flex items-center justify-center p-4 bg-slate-100 dark:bg-slate-950 text-slate-900 dark:text-slate-100 font-sans transition-colors duration-200 relative">
        ...
        <div className="absolute top-4 right-4">
          <button
            ...
            className="p-2.5 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-600 dark:text-amber-300 shadow-sm"
          >
        ...
        <div className="w-full max-w-md bg-white dark:bg-slate-900 rounded-3xl p-8 shadow-2xl border border-slate-200 dark:border-slate-800 space-y-6">
          ...
          <div className="text-center space-y-2">
            <div className="w-14 h-14 rounded-2xl bg-sky-600 text-white mx-auto flex items-center justify-center shadow-lg shadow-sky-600/30">
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400">
              Acesso exclusivo à equipe de saúde credenciada
            </p>
          </div>

          {/* Mensagem de Erro de Autenticação */}
          {loginErro && (
            <div className="p-3 text-xs bg-rose-50 dark:bg-rose-950/60 border border-rose-300 dark:border-rose-800 text-rose-700 dark:text-rose-300 rounded-xl flex items-center gap-2 animate-in fade-in duration-200">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full pl-9 pr-3 py-2.5 text-sm rounded-xl border border-slate-300 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/60 text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-sky-500 focus:outline-none"
                />
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full pl-9 pr-10 py-2.5 text-sm rounded-xl border border-slate-300 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/60 text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-sky-500 focus:outline-none"
                />
                <Lock className="w-4 h-4 text-slate-400 absolute left-3 top-3" />
                <button
                  type="button"
                  onClick={() => setMostrarSenha(!mostrarSenha)}
                  className="p-1 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 absolute right-3 top-2.5"
            ...
            <button
              ...
              className="w-full py-3 rounded-xl bg-sky-600 hover:bg-sky-700 disabled:opacity-60 text-white font-bold text-sm shadow-md shadow-sky-600/20 transition-all active:scale-95 flex items-center justify-center gap-2"
            >
              {loginLoading ? (
                <>
          ...
          <div className="pt-2 border-t border-slate-100 dark:border-slate-800 text-center">
            <p className="text-[11px] text-slate-400 dark:text-slate-500 flex items-center justify-center gap-1.5">
              ...
              <span>Acesso restrito • Credenciais gerenciadas pela administração</span>
            </p>
          </div>
        </div>
      </div>
      ...
      <header className="bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 sticky top-0 z-30 shadow-sm transition-colors duration-200">
        <div className="max-w-6xl mx-auto px-4 py-3 flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            {/* Usuário Autenticado / Profissional Responsável */}
            <div className="flex items-center gap-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 px-3 py-1.5 rounded-xl shadow-sm">
              <div className="w-6 h-6 rounded-full bg-sky-600 text-white flex items-center justify-center text-xs font-bold shadow-sm">
                {usuarioLogado?.nome ? usuarioLogado.nome[0].toUpperCase() : 'U'}
              </div>
              <div className="flex flex-col text-left">
                <div className="flex items-center gap-1.5">
                  <span className="text-xs font-bold text-slate-800 dark:text-slate-200 max-w-[120px] truncate">
                    {usuarioLogado?.nome || autor}
                  </span>
                  <span className="text-[9px] font-bold px-1.5 py-0.2 bg-emerald-100 dark:bg-emerald-950 text-emerald-700 dark:text-emerald-400 rounded-md border border-emerald-200 dark:border-emerald-800">
                    {usuarioLogado?.role || 'EQUIPE'}
                  </span>
                </div>
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-950/60 hover:text-rose-600 border border-slate-200 dark:border-slate-700 text-xs font-bold transition-all active:scale-95 shadow-sm"
            >
